...
public class BestTimetoBuyandSellStockIV {
        ...
        for (int i = 0; i < prices.length; i++) {
            for (int j = i + 1; j < prices.length; j++) {
                ...
                if (map.containsKey(i)) {
                    map.get(i).add(new int[] {i, j, prices[j] - prices[i]});
                } else {
                    List<int[]> l = new ArrayList<>();
                    l.add(new int[] {i, j, prices[j] - prices[i]});
                    map.put(i, l);
                }
            }
        }
            ...
            for (var p : e.getValue()) {
                ...
                while (!q.isEmpty()) {
                    ...
                    for (int i = 0; i < size; i++) {
                        var pp = q.poll();
                        val = Math.max(val, p[2]);
                        if (trans > 0) {
                            for (int j = pp[1] + 1; j < prices.length; j++) {
                                if (map.containsKey(j)) {
                                    for (var ppp : map.get(j)) {
                                        int[] t = new int[] {ppp[0], ppp[1], ppp[2] + pp[2]};
                                        q.add(t);
                                    }
                                }
                            }
    ...
    // https://leetcode.com/problems/best-time-to-buy-and-sell-stock-iv/discuss/2555893/JAVA-oror-Easy-Solution-with-Explanation-oror-Beginner-Friendly
    ...
    public static void main(String[] args) {
        ...
        System.out.println(b.maxProfit(29, new int[]
